...
| v2.3 · 首批真实用户反馈迭代 | 08-27/28 | 第一批内测用户反馈驱动：修复 iOS 无声（手势音频解锁）、VIP 歌曲 30 秒截断（只交付完整曲目）、标题可读性、配色软化（默认暮色皮肤）、移动端性能降载。详见 [USER_FEEDBACK.md](USER_FEEDBACK.md) |
| v2.4 · 第二批反馈 + 供应链修复 | 08-28 | 标题改「说说此刻的心情」、加载超时明示+一键重试、进度条改真 scrubber、界面去术语化（V/A 坐标撤下）、并发三层加固（chips 缓存/限流重试/队列）；修复 LLM 供应链事故（欠费→Kimi 并发限制→DeepSeek v4-flash，推断从 2 分钟降到 4 秒）；氛围感输入实测 4/4 通过 |
