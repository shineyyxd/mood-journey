...
## v2.5 · 去噪音：两个决策点合并为一个（2026-08-30）

**操作逻辑变化**：分歧屏（强制二选一）删除。新流程：输入 → 纱幕「正在接住你…」→ **确认屏**——「接住你了：{情绪词}」+ LLM 共情文案 +「带你去 → {推荐终点}」，3 秒自动出发；备选终点降级为一行「或者去这里 →」链接，点它才换。单候选用户同样过确认屏（预期管理对所有人成立）。入口再删预设 chips 排；旅程态新增「感觉不准？」小字出口；盘面拖拽默认禁用，仅 HUD 唤起时可拖。
**对用户的意义**：全程只做一次主动动作（说心情），其余被温柔地带着走——「无法预期输出」和「决策摩擦」两个反馈一次解掉。

**同期 P0 修复（第三批用户反馈）**：音频代理支持 Range 206 续传（iOS 暂停恢复/拖拽/切歌加载从根上修复）；流地址改为按歌曲 ID 现解析（网易云 URL 20 分钟过期导致的"切歌死"根治）。

![确认屏](v2.5-去噪音/confirm.png)

## v2.5.1（2026-08-30）

- **盘面拖拽恢复**：v2.5 的「仅 HUD 可拖」门控取消，恢复默认可拖拽改情绪点；加 6px 位移阈值防快速划过/误触。
- **曲库扩容（VIP cookie）**：接入产品所有者自有网易云 VIP 账号（服务器 .env 注入，不进镜像/git），VIP 歌（fee=1/4）不再被跳过、全曲可播；防 30 秒试听截断兜底保留。勘察结论：周杰伦曲目为平台级无版权（2018 年起），VIP 亦不可播，非账号问题。
